...
console.log('🔍 routes/orders.js - PaymentSession model loaded:', typeof PaymentSession === 'function' ? '✅' : '❌');
...
router.post('/', async (req, res, next) => {
  try {
    const { items, orderType, notes, paymentSessionId } = req.body;
    ...
    if (!paymentSessionId) {
      throw new ValidationError('Payment session ID is required. Order must be paid first.');
    }
    ...
    if (!paymentSession) {
      throw new ValidationError('Invalid payment session. Please restart checkout.');
    }
    
    // Check if payment session is already used
    if (paymentSession.orderCreated === true) {
      throw new ValidationError('Order already created for this payment session.');
    }
    
    // Check payment status
    if (paymentSession.status !== 'approved') {
      throw new ValidationError(`Cannot create order: Payment status is '${paymentSession.status}'. Payment must be approved first.`);
    }
    ...
    if (Math.abs(paymentSession.amount - calculatedGrandTotal) > 0.01) {
      logger.warn(`Amount mismatch: session $${paymentSession.amount} vs order $${calculatedGrandTotal}`);
    ...
    logger.info(`Order created after payment approval: #${orderNumber} for session ${paymentSessionId}`);
    
    // Create print jobs (kitchen, bar, customer)
    await createPrintJobs(orderId, orderNumber, orderType, items, notes || '', subtotal, tax, totalPrice);
